  ...
  if (!userId) {
    ...
    try {
      ...
      if (images.length > 0) {
        res
          .status(200)
          .send({ data: images, message: "Lấy hình ảnh thành công" });
      } else {
    ...
    try {
      ...
      res
        ...
        .send({
          data: images,
          message: "Lấy hình ảnh theo người dùng thành công",
        });
    } catch (error) {
      console.log(error);
      res.status(500).send("Bất ổn gòi BE");
...
const updateImg = async (req, res) => {
  ...
  if (image.nguoi_dung_id !== Number(userId)) {
    return res
      .status(403)
      .send("Bạn không có quyền chỉnh sửa hình ảnh của người khác.");
  }
  try {
    await Image.update({
      where: {
...
const uploadImg = async (req, res) => {
  try {
    ...
    fs.readFile(process.cwd() + imageUrl + file.filename, (err, data) => {
      ...
      const fileBase = `data:${file.mimetype};base64,${Buffer.from(
        data
      ).toString("base64")}`;
...
const deleteImg = async (req, res) => {
  try {
    ...
    if (image.nguoi_dung_id !== Number(userId)) {
      res
        .status(400)
        .send("You are not authorized to alter the photo of another user.");
      return;
    }
    await Image.delete({
      where: {
